module AddressesHelper
  def format_address(addr)
    ...
    add << "<span property='addressRegion'>#{addr.town.county.name}</span><br/>" if addr.town.county.name.present?
    add << "<span property='postalCode'>#{addr.postcode}</span><br/>#{addr.dx}</p>"
