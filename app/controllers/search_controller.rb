    ...
    if @search
      ...
      if postcode =~ @search
        ...
        if service_available
          ...
          # @results = Court.within_radius(mile_in_meters, latlon[0]['lat'], latlon[0]['long']).all # activerecord-postgres-earthdistance method
          @results = Court.near([latlon[0]['lat'], latlon[0]['long']], @distance, :order => :distance).paginate(:page => page, :per_page => per_page)
        else
          flash[:notice] = "Post code search unavailable"
        end

      else
        @results = Court.search(@search, page, per_page)
      end

    else
      @results = Court.order(:name).paginate(:page => page, :per_page => per_page)
    end
