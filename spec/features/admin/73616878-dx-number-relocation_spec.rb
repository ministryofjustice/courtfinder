  ...
  context 'legal professional user' do
    let!(:court) do
      create(:court, name: 'the court') do |court|
        court.contacts << create(:contact, telephone: '2343', contact_type_id: ContactType.find_by(name: 'DX').id, court_id: court.id)
        court.contacts << create(:contact, telephone: '01 1234 56678', contact_type_id: ContactType.find_by(name: 'telephone').id, court_id: court.id)
      end
    end
