class CourtSerializer
  ...
  def serialize_court
    hash = {
      'name'          => @court.name,
      'slug'          => @court.slug,
      'updated_at'    => @court.graph_updated_at,
      'update_type'   => 'major',
      'locale'        => 'en',
      'closed'        => false,
      'alert'         => @court.alert,
  ...
  def serialize_array(collection)
    collection = self.send(collection)
    array = []
    serialization_method = "serialize_#{collection}".to_sym
    collection.each { |c| array << @court.send(serialization_method(c)) }
    array
  end
  ...
  def serialize_opening_times(opening_time)
    {
      ...
      'description' => opening_time.description
    }
  end

  def serialize_adresses(address)
